...
test.describe('Delivery Process', () => {
    ...
    test('Display correct collection point in order based on postal code', async ({ page }) => {
        ...
        const firstOption = await page.$eval('#pickup-location option:first-child', el => el.textContent.trim());
        expect(firstOption).toContain('Happiness Printing Store, 304 Orchard Road, #04-20, 238863 - 0.6 km');
        ...
        const isPickupChecked = await page.$eval('#pickup-option', el => el.checked);
