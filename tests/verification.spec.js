...
test.describe("Profile Update with Password Verification", () => {
  test("Update email", async ({ page }) => {
    const USERS = [
      {
        ...
        password: "123",
        newEmail: "lily@gmail.com",
      },
      {
        email: "lily@gmail.com",
        password: "123",
        newEmail: "yisang@gmail.com",
      },
    ];
    ...
    await page.evaluate(() => {
      document.querySelector("#password-prompt-box").style.display = "block";
    });
    ...
    await page.waitForResponse(
      (response) =>
        response.url().includes("/profile/email/update") &&
        response.status() === 200
    );
